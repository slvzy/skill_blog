...
/*func GetOauthTokenByToken(token string) (ot *OauthToken) {
	ot = new(OauthToken)
	datasource.DB.Where("token =  ?", token).First(&ot)
	return
}*/
...
/*func UpdateOauthTokenByUserId(userId uint) (ot *OauthToken) {
	datasource.DB.Model(ot).Where("revoked = ?", false).Where("user_id = ?", userId).Updates(map[string]interface{}{"revoked": true})
	return
}*/
